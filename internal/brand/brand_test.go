...
func TestIcons(t *testing.T) {
	...
	if icons[1]["src"] != "https://acahti.example/acahti.svg" || icons[1]["mimeType"] != "image/svg+xml" {
		t.Fatalf("svg=%v", icons[1])
	}
	if icons[2]["src"] != "https://acahti.example/acahti.png" || icons[2]["mimeType"] != "image/png" {
		t.Fatalf("png=%v", icons[2])
	}
