...
1. `GET /ui/pipelines` — newest run per `(repo, ref)` (`created DESC`). A later number on the same branch replaces earlier ones. Visibility from the org catalog. Rows use stored `jobs` (no YAML fetch).
...
| Request | Returns |
|---|---|
| `GET /ui/pipelines` | latest run per `(repo, ref)` with stored `jobs` |
