...
Org members who can see repos can install. Publish uses the triggering user's identity (`npm-publish` / `pypi-publish` / `pkg_publish`). Those pipes install, build, and upload in a container; YAML does not `docker run` or write `.npmrc` / `.netrc`.
