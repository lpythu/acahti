...
Upgrade the island: bump `ACAHTI_VERSION`, push `main`, `bash scripts/tag-release.sh`. The host self-hosted runner (`labels: acahti`) checks out that tag, rsyncs into `/home/saidc/acahti` (keeps `.env`), and runs `scripts/up.sh` (never `compose down -v`). Then re-run `agent.sh` on buildof if `WOODPECKER_VERSION` changed.
